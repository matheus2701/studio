    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          {appointmentToEdit 
            ...
            : "Selecione o melhor dia e horário para o atendimento."}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        <Card className="md:col-span-5 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg">1. Escolha a Data</CardTitle>
          </CardHeader>
        ...
        <Card className="md:col-span-7 shadow-sm">
          <CardHeader>
            <CardTitle className="text-lg">2. Serviços e Horários</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            {selectedDate ? (
                          ...
                          <Label htmlFor={`proc-${proc.id}`} className="text-sm font-medium cursor-pointer flex-1">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-primary font-bold">
                                R$ {proc.isPromo && proc.promoPrice !== undefined ? proc.promoPrice.toFixed(2) : proc.price.toFixed(2)}
                              </span>
                            </div>
                ...
                {selectedProcedureIds.length > 0 && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    {availableTimeSlots.length > 0 ? (
                      ...
                      <div className="p-4 border border-dashed rounded-lg text-center text-sm text-muted-foreground bg-muted/5">
                        Não há horários para esta combinação no dia selecionado.
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex flex-col items-center justify-center py-10 text-muted-foreground">
                <CalendarCheck2 className="h-10 w-10 mb-2 opacity-20" />
      ...
      {selectedDate && selectedProceduresDetail.length > 0 && selectedTime && (
        <Card className="border-primary/20 shadow-lg animate-in zoom-in-95 duration-300">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <div>
              <CardTitle className="text-xl">3. Confirmar Detalhes</CardTitle>
              <CardDescription>Finalize o agendamento preenchendo os dados do cliente.</CardDescription>
            </div>
            {appointmentToEdit && (
        ...
        <AlertDialogContent className="sm:max-w-[420px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center pt-2">
              O horário foi reservado e agora pode ser visualizado na sua agenda de atendimentos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0 mt-6">
            <Button variant="outline" onClick={handleGoToSchedule} className="flex-1">
              <ListTodo className="mr-2 h-4 w-4" /> Ver na Agenda
            </Button>
            <Button onClick={handleCreateAnother} className="flex-1">
              <PlusCircle className="mr-2 h-4 w-4" /> Criar Outro
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
